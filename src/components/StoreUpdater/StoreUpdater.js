...
import "bulma/bulma.sass";
  ...
  const onUpdate = (item) => {
    dispatch(
      updateListItemAsyncAction({ value: item.value, isLoading: true }, 0)
    );
    dispatch(
      updateListItemAsyncAction({ value: item.value, isLoading: false }, 2000)
    );
  };
    ...
    <div className="store-updater container">
      ...
      {list.length > 0 && (
        <div className="list columns is-multiline is-flex-wrap-wrap mt-6">
          {list.map((item) => (
            <div
              className="card column is-4 m-0 has-text-centered"
              key={item.value}
            >
              <div className="content">{item.value}</div>
              <div style={{ height: "25px" }}>
                {item.isLoading && "Updating..."}
              </div>
